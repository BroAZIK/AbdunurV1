import speech_recognition as sr
from gtts import gTTS
import os, pyttsx3
from opening_apps.auto_selenium import *
from playsound import playsound

def speak(text):
    engine = pyttsx3.init()
    voices = engine.getProperty('voices')
    engine.setProperty('voice', voices[1].id)
    engine.say(text)
    engine.runAndWait()

def listen():
    r = sr.Recognizer()
    with sr.Microphone() as source:
        print("Gapiring...")
        audio = r.listen(source)
        try:
            text = r.recognize_google(audio, language="uz-UZ")
            print(f"Siz aytdingiz: {text}")
            return text.lower()
        except sr.UnknownValueError:
            print("Kechirasiz, ovozingizni tanib bo'lmadi.")
            return ""
        except sr.RequestError:
            print("Xatolik yuz berdi!")
            return ""

def chatbot_response(user_input):
    print(user_input)

    if 'salom' in user_input or 'qandaysan' in user_input:
        playsound('audio/ttsvoices/Salom.mp3')
    elif 'isming nima' in user_input:
        playsound('audio/ttsvoices/ismim_Bobur.mp3')
    
    elif "youtubeni och" in user_input or "yutubga kir" in user_input or "yutubni och" in user_input:
        return open_youtube()
    elif "deb qidir" in user_input:
        text = user_input.replace("deb qidir", "")
        return search_youtube(text)
    elif "telegramni och" in user_input or "telegramga kir" in user_input:
        return open_telegram()
    elif "deb qidir" in user_input and "telegramdan" in user_input:
        text = user_input.replace("ni qidir", "").replace("telegramdan", "")
        return search_telegam(text)
    elif 'rahmat' in user_input:
        return "Arzimaydi!"
    elif 'andijondan' in user_input:
        return 'Andijon va Qoraqalpogʻiston o‘rtasidagi masofa taxminan 900-950 kilometrni tashkil qiladi.'
    elif "xayr" in user_input or 'exit' in user_input:
        return "Xayr, keyingi safar ko'rishguncha!"
    else:
        return "Afsus, bu savolga javobim yo'q."

if __name__ == "__main__":
    # speak("Salom! Men bilan gaplashishingiz mumkin.")
    while True:
        user_input = listen()
        if user_input:
            response = chatbot_response(user_input)
            speak(response)
        if 'xayr' in user_input or 'exit' in user_input:
            break
